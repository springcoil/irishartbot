...
#I dunno if I need ssm just yet
h = HTMLParser()

def lambda_handler(event, context):
    bucket_name = 'irish-art-bot'
    ...
    # Shuffle images
    single_image_meta = random.choice(list(indexed_json.items()))

    url = single_image_meta[0]
    painter = single_image_meta[1][0]
    title= single_image_meta[1][1]
    year = single_image_meta[1][2]
        ...
        for i in range(0, 3):
                try:
                    ...
                    with open(path, 'rb') as img:
                        ...
                        twitter.update_status(status="\"%s\"\n%s, %s\n#modernart #art" % (title, painter, year),
